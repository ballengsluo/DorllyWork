...
<body>
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 工单管理 <span class="c-gray en">&gt;</span> 工单列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:2px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div id="list" class="pt-5 pr-20 pb-5 pl-20">
	    <div class="cl pd-5 bg-1 bk-gray mt-5"> 
		    ...
		    客户名称&nbsp;<input type="text" class="input-text size-S" style="width:110px" readonly id="CustNameS" name="">
              <a href="javascript:void();" class="btn btn-primary radius upload-btn size-S" onclick="ChooseCust('CustNoS')" >选择</a>
	    ...
	    <div class="cl pd-5 bg-1 bk-gray mt-5"> 
            <span class="l">
                <a href="javascript:;" onclick="insert()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加工单</a>
                <a href="javascript:;" onclick="update()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe60c;</i> 修改工单</a> 
                <a href="javascript:;" onclick="del()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 删除工单</a> 
                <a href="javascript:;" onclick="view()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe6c6;</i> 查看工单</a> 
                <a href="javascript:;" onclick="print()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe652;</i> 打印工单</a> &nbsp;
                <a href="javascript:;" onclick="exportexcel()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe645;</i> 导出工单</a> &nbsp;
                <a href="javascript:;" onclick="confirm()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe6e1;</i> 确认销单</a> 
                <a href="javascript:;" onclick="adduser()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe607;</i> 重新指派</a> 
      ...
      <div class="form form-horizontal" id="editlist">
        ...
        <div class="row cl">
          ...
          <div class="formControls col-4">
              <input type="text" class="input-text required upload-url" id="CustName" readonly style="width:320px;" data-valid="between:0-200" data-error="客户名称长度为0-200位" />
              <a href="javascript:void();" class="btn btn-primary radius upload-btn" onclick="ChooseCust('CustNo')" >选择</a>
        ...
        <div class="row cl">
          ...
          <div class="formControls col-3">
              <input type="text" class="input-text upload-url" id="RegionName" placeholder="地区" readonly style="width:200px;" />
        ...
        <div class="row cl">
          ...
          <div class="formControls col-2">
            <input type="text" class="input-text required" placeholder="销售订单号" id="SaleNo" data-valid="between:0-30" data-error="销售订单号长度为0-30位" disabled="disabled" />
          </div>
          <div class="col-3"></div>
        </div>
        <div class="row cl">
    ...
    <script type="text/javascript">
        function BandResuleData(temp) {
            ...
            if (vjson.type == "print") {
                stat = 0;
                if (vjson.flag == "1") {
                    document.getElementById("openfile").src = "../../pdf/" + vjson.path;
                    window.open("../../pdf/" + vjson.path);
                }
                else {
            ...
            if (vjson.type == "getDeptUser") {
                if (vjson.flag == "1") {
                    $("#AlloDept").val(vjson.DeptNo);
                    $("#AlloDeptName").val(vjson.DeptName);
                    $("#TreatUser").val(vjson.UserNo);
                    $("#TreatUserName").val(vjson.UserName);
            ...
            if (vjson.type == "getNewMessage") {
                if (vjson.flag == "0") {
                    $('embed').remove();   
                    $('body').append('<embed src="../../images/msg.wav" autostart="true" hidden="true" loop="false">');
                }
                window.setTimeout("minute()", 5 * 1000);
                return;
            }
            if (vjson.type == "exportexcel") {
                if (vjson.flag == "0") {
        ...
        function getDeptUser() {
            if ($("#OrderType").val() == "" || $("#RegionNo").val() == "") return;

            var submitData = new Object();
            submitData.Type = "getDeptUser";
            submitData.OrderType = $("#OrderType").val();
            submitData.RegionNo = $("#Region").val();
            transmitData(datatostr(submitData));
            return;
        }
        ...
        function ChooseCust(id) {
            var temp = getRootPath().replace("/pm", "").replace("/paltform", "") + "/pm/Base/ChooseCust.aspx?id=" + id;
            layer_show("选择页面", temp, 560, 480);
        }
