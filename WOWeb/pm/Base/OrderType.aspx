<%@ Page Language="C#" AutoEventWireup="true" Inherits="project.Presentation.Base.OrderType,project"  %>
<!DOCTYPE>
...
<body>
    ...
    <div>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 基础资料 <span class="c-gray en">&gt;</span> 工单类型 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:2px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    ...
    <div id="edit" class="pt-5 pr-20 pb-5 pl-20" style="display:none;">
      <div class="form form-horizontal" id="editlist">
        <div class="row cl">
          <label class="form-label col-2"><span class="c-red">*</span>工单类型编号：</label>
          <div class="formControls col-4">
            <input type="text" class="input-text required" placeholder="工单类型编号" id="OrderTypeNo" data-valid="isNonEmpty||between:1-30" data-error="工单类型编号不能为空||工单类型编号长度为0-30位"/>
          </div>
          <div class="col-3"></div>
        </div>
        <div class="row cl">
          <label class="form-label col-2"><span class="c-red">*</span>工单类型名称：</label>
          <div class="formControls col-4">
            <input type="text" class="input-text required" placeholder="工单类型名称" id="OrderTypeName" data-valid="isNonEmpty||between:1-50" data-error="工单类型编号不能为空||工单类型编号为0-50位"/>
          </div>
          <div class="col-3"></div>
        </div>
        <div class="row cl">
          <label class="form-label col-2"><span class="c-red">*</span>流程：</label>
          <div class="formControls col-4">
              <%=flow %>
          </div>
          <div class="col-3"></div>
    ...
    <script type="text/javascript">
        function BandResuleData(temp) {
            ...
            if (vjson.type == "submit") {
                if (vjson.flag == "1") {
                    ...
                    showMsg("OrderTypeNo", "工单类型在系统已存在", "1");
                }
                else {
                    layer.alert("数据操作出错！");
                }
            ...
            if (vjson.type == "update") {
                if (vjson.flag == "1") {
                    $("#OrderTypeNo").val(vjson.OrderTypeNo);
                    $("#OrderTypeName").val(vjson.OrderTypeName);
                    $("#FlowNo").val(vjson.FlowNo);

                    $("#OrderTypeNo").attr("disabled", true);
        ...
        function insert() {
            ...
            id = "";
            type = "insert";
            $("#OrderTypeNo").attr("disabled", false);
            $("#OrderTypeNo").val("");
            $("#OrderTypeName").val("");
            $("#FlowNo").val("");
        ...
        function submit() {
            if ($('#editlist').validate('submitValidate')) {
                ...
                submitData.id = id;
                submitData.OrderTypeNo = $("#OrderTypeNo").val();
