(function(factory) {
    ...
    Validator.prototype.showMsg = function(target, msg, status, callback) {
        ...
        var $context = target.parent().next();
        var $msg = $context.find('.valid_message');
        var _other = target.attr('data-type') || '';
        $msg.remove();
        if (status == 3) {
            target.removeClass('success tip error').addClass('tip ' + _other);
        }
        else {
            if (tiptype == "1")
                $context.append('<span class="valid_message ' + _current + '_span">' + msg + '</span>');
            target.removeClass('success tip error').addClass(_current + ' ' + _other);
        }
    };
...
function showMsg(id,msg,type) {
    ...
    var $context = $("#" + id).parent().next();
    var $msg = $context.find('.valid_message');
    var _other = $("#" + id).attr('data-type') || '';
    $msg.remove();
    if (type=="1")
        $context.append('<span class="valid_message error_span">' + msg + '</span>');
    $("#" + id).removeClass('success tip error').addClass('error ' + _other);
